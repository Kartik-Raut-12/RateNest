...
const Register = () => {
  ...
  const validate = () => {
    ...
    if (!form.password) e.password = 'Password is required'
      ...
      e.password = 'Must be 8–16 chars with 1 uppercase and 1 special character'
    setErrors(e)
  ...
  const handleSubmit = async (e: { preventDefault(): void }) => {
    ...
    try {
      ...
      login(token, user)
      toast.success('Account created! Welcome to RateNest.')
      navigate('/stores')
    } catch (err: unknown) {
      const error = err as { response?: { data?: { error?: string; errors?: { message: string }[] } } }
      const msg = error.response?.data?.error || error.response?.data?.errors?.[0]?.message || 'Registration failed'
      toast.error(msg)
    } finally {
      setLoading(false)
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-slate-50 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-[400px]">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1.5">Join RateNest and start rating stores</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className={`text-xs ${nameLen >= 20 && nameLen <= 60 ? 'text-emerald-500' : nameLen > 0 ? 'text-orange-400' : 'text-gray-400'}`}>
                  {nameLen}/60
                </span>
              </div>
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all ${
                  errors.name ? 'border-red-400 bg-red-50/40' : 'border-gray-200 hover:border-gray-300'
                }`}
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all ${
                  errors.address ? 'border-red-400 bg-red-50/40' : 'border-gray-200 hover:border-gray-300'
                }`}
            ...
            <div>
              ...
              {errors.password
                ...
                : <p className="text-gray-400 text-xs mt-1.5">8–16 chars · uppercase · special character</p>
              }
            </div>
